        ...
        jgQuery(openGraph).then(function(response, err) {
          ...
          jgQuery(resetGraph).then(function(response, err) {
            ...
            jgQuery(initialise).then(function(response, err) {
              var wordsInfo = JSON.parse(response);
...
initialise +=
  "def d1 = graph.addVertex(T.label, 'definition', 'text', 'a greeting');";
initialise +=
  "def d2 = graph.addVertex(T.label, 'definition', 'text', 'a planet');";
...
function listWords() {
  return new Promise(function(resolve, reject) {
    ...
    jgQuery(gremlinQuery)
      .then(function(response, err) {
        if (err) {
          console.log("error in listWords");
          reject(err);
        }
        let wordsInfo = JSON.parse(response);
...
function getWords(request) {
  return new Promise(function(resolve, reject) {
    let gremlinQuery = traverseGraph;
    gremlinQuery +=
      "def wQuery=g.V().has('text', '" + request.query.word + "').next();";

    // use the connected components recipe (http://tinkerpop.apache.org/docs/current/recipes/#connected-components)
    gremlinQuery +=
      "g.V(wQuery).emit(cyclicPath().or().not(__.in())).repeat(__.in()).until(cyclicPath()).aggregate('p').by(path()).cap('p').unfold().limit(local, 1).dedup().map(__.as('v').select('p').unfold().filter(unfold().where(eq('v'))).unfold().dedup().order().by(id).fold()).dedup().path();";
...
function checkEdge(vertices) {
  ...
  return new Promise(function(resolve, reject) {
    let gremlinQuery = traverseGraph;
    gremlinQuery +=
      "def eQuery=g.V().hasLabel('word').has('text', '" +
      vertices.word1 +
      "').in('" +
      vertices.edgeType +
      "').has('text', '" +
...
function addVertex(vertex) {
  return new Promise(function(resolve, reject) {
    ...
    gremlinQuery +=
      "def v1 = graph.addVertex(T.label, '" +
      vertex.label +
      "', 'text', '" +
      vertex.text +
      "');";
...
function addEdge(vertices) {
  return new Promise(function(resolve, reject) {
    ...
    gremlinQuery +=
      "def w1=g.V().hasLabel('word').has('text', '" +
      vertices.word1 +
      "').next();";
    gremlinQuery +=
      "def w2=g.V().has('text', '" + vertices.word2 + "').next();";
    gremlinQuery +=
      "w2.addEdge('" + vertices.edgeType + "', w1, 'weight', 0.5d);";
    gremlinQuery += "graph.tx().commit();";
    jgQuery(gremlinQuery)
      .then(function(response) {
        let wordsInfo = JSON.parse(response);
...
app.put("/words", function(request, response) {
  ...
  let word1Label = "word";
  let word2Label =
    request.body.edgeType === "definition" ? "definition" : "word";
  checkVertex({ label: word1Label, text: request.body.word1 })
    .then(function(resp) {
      console.log(resp);
      checkVertex({ label: word2Label, text: request.body.word2 }).then(
        function(resp) {
          console.log(resp);
